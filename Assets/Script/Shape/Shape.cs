...
public class Shape : MonoBehaviour, IPointeClickHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerDownHandler
{
    ...
    public void DeactivateShape()
    {
        if (_shapeActive)
        {
            foreach (var square in _currentShape)
            {
                square?.GetComponent<ShapeScuare>().DeactivateShape();
            }
        }
    ...
    public void ActivateShape()
    {
        if (!_shapeActive)
        {
            foreach (var square in _currentShape)
            {
                square?.GetComponent<ShapeScuare>().Activateshape();
            }
        }
    ...
    public void CreateShape(ShapeData shapeData)
    {
        ...
        // establecer la posición para formar la forma final
    ...
    private float GetXPositionForShapeSquare(ShapeData shapeData, int column, Vector2 moveDistance)
    {
        ...
        if (shapeData.columns > 1)//calcular la posicion vertical
        {
            if (shapeData.columns % 2 != 0)
            {
                var middleSquareIndex = (shapeData.columns - 1) / 2;
                var multiplier = (shapeData.columns - 1) / 2;
                if (column < middleSquareIndex)// mover en numero negativo
                {
                    shiftOnX = moveDistance.x * -1;
                    shiftOnX *= multiplier;
                }
                else if (column > middleSquareIndex)// moverse a positivo
                {
                    shiftOnX = moveDistance.x * 1;
                    shiftOnX *= multiplier;
                }
            ...
            else
            {
                ...
                if (column < middleSquareIndex1 && column < middleSquareIndex2)// mueve en negativo
                {
                    shiftOnX = moveDistance.x * -1;
                    shiftOnX *= multiplier;
                }
